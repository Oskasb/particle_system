	...
	RibbonMesh.prototype.updateStep = function(trailData, camPos, index) {
		...
		for (var i = 0; i < this.segmentCount; i++) {
			...
			if (this.facingMode == 'Billboard') {
				...
				trailDirection.normalize().mul(trailSegmentData.width * 0.5);
			} else if (trailSegmentData.tangent != null) {
				trailDirection.set(trailSegmentData.tangent).mul(trailSegmentData.width * 0.5);
			} else {
				trailDirection.set(trailSegmentData.width * 0.5, 0, 0);
			}
	...
	RibbonMesh.prototype.updateInterpolate = function(trailData, camPos, index, tpf) {
		...
		for (var i = 0; i < this.segmentCount; i++) {
			...
			if (this.facingMode == 'Billboard') {
				...
				trailDirection.cross(trailCamVec);
				trailDirection.normalize().muld(trailWidth, trailWidth, trailWidth);
				...
				trailDirection.setv(trailSegmentData.tangent).mul(trailWidth);
			} else {
				trailDirection.setd(trailWidth, 0, 0);
			}
